    ...
    $user_id = $currentEnroll['user_id'];

    $stmt = $conn->query("SELECT roles.role as role, users.role_id as role_name
    FROM roles
    LEFT JOIN users ON roles.id=users.role_id
    WHERE users.id=$user_id");
    $userRoll = $stmt->fetch(PDO::FETCH_ASSOC);
    ...
    if (isset($_POST['course_id'])) {
        ...
        $addData = $conn->exec("UPDATE enrollments SET user_id =$user_id, course_id=$course_id, is_active=$is_active, enrolled_at=now()  where id=$id");
        if ($addData) {
            ...
            <div class=" card-content">
                <form method="POST" class="flex flex-col space-y-4" action="">
                    ...
                    <div class="control">
                        ...
                        <input readonly type="text" name="role_id" value="<?php echo $userRoll['role'] ?? null ?>" class="input" />
                    ...
                    <div class="control">
                        ...
                        <select
                            ...
                            name="is_active"
                            id="is_active">
                            <option value="1" <?php if ($currentEnroll['is_active'] == 1) {
                                                    echo 'selected';
                                                } ?>>Active</option>
                            <option value="0" <?php if ($currentEnroll['is_active'] == 0) {
                                                    echo 'selected';
                                                } ?>>Inactive</option>
                        </select>
                    </div>
